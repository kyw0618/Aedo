...
import com.example.my_heaven.util.`object`.Constant.SEARCH_BURIED
import com.example.my_heaven.util.`object`.Constant.SEARCH_COFFIN_DATA
import com.example.my_heaven.util.`object`.Constant.SEARCH_DECEASED_NAME
import com.example.my_heaven.util.`object`.Constant.SEARCH_DOFP_DATA
import com.example.my_heaven.util.`object`.Constant.SEARCH_EOD_DATA
import com.example.my_heaven.util.`object`.Constant.SEARCH_PLACE
import com.example.my_heaven.util.`object`.Constant.SEARCH_RELATION_NAME
import com.example.my_heaven.view.main.detail.search.SearchDetailActivity
...
class SearchAdapter (private val searchlist : List<CreateSearch>, private val context: Context)
    ...
    inner class ViewHolder (itemView: View? ) : RecyclerView.ViewHolder(itemView!!){
        ...
        fun bind(list: CreateSearch?, context: Context) {
            search_place?.text = list?.place?.place_name
            ...
            cl_search?.setOnClickListener {
                ...
                intent.putExtra(SEARCH_EOD_DATA,list?.eod?.date)
                intent.putExtra(SEARCH_DECEASED_NAME,list?.deceased?.name)
                intent.putExtra(SEARCH_COFFIN_DATA,list?.coffin?.date)
                intent.putExtra(SEARCH_DOFP_DATA,list?.dofp?.date)
                intent.putExtra(SEARCH_BURIED,list?.buried)
                intent.putExtra(SEARCH_PLACE,list?.place?.place_name)
